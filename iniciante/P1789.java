 ...
 * contém um inteiro L (1 ≤ L ≤ 500) representando o número de lesmas do grupo, e a segunda linha contém
 * L inteiros Vi (1 ≤ Vi ≤ 50) representando as velocidades de cada lesma do grupo.
 *
 * A entrada termina com o fim do arquivo (EOF).
 *
 * Saída
 * Para cada caso de teste, imprima uma única linha indicando o nível de velocidade da lesma mais veloz
 * do grupo.
 */
...
public class P1789 {
    public static void main(String[] args) {
        ...
        while (in.hasNext()) {
            int melhor = 0;
            int lesmas = in.nextInt();

            for (int i = 0; i < lesmas; i++) {
                int velocidade = in.nextInt();

                melhor = Math.max(melhor, nivelLesma(velocidade));
            }
    ...
    public static int nivelLesma(int velocidade) {
        if (velocidade < 10)
            return 1;
